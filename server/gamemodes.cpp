...
int CGameMode::OnPlayerCommandText(cell playerid, unsigned char * szCommandText)
{
	...
	if(!pNetGame->GetPlayerPool()->GetSlotState((WORD)playerid))
		return (int)ret;
...
int CGameMode::OnRconLoginAttempt(char* ip, char* password, cell success)
{
	CHECK_INIT();
	...
	if (!amx_FindPublic(&m_amx, "OnRconLoginAttempt", &idx))
	{
		...
		amx_PushString(&m_amx, &amx_addr1, &phys_addr, (char*)password, 0, 0);
		amx_PushString(&m_amx, &amx_addr2, &phys_addr, (char*)ip, 0, 0);
		amx_Exec(&m_amx, &ret, idx);
...
int CGameMode::OnDialogResponse(cell playerid, cell dialogid, cell response, cell listitem, char* inputtext)
{
	CHECK_INIT();
	int idx;
	cell ret = 0;
	...
	{
		cell amx_addr, *phys_addr;
		amx_PushString(&m_amx, &amx_addr, &phys_addr, (char*)inputtext, 0, 0);
...
int CGameMode::OnUnoccupiedVehicleUpdate(cell vehicleid, cell playerid, cell passenger_seat, VECTOR vecPos, VECTOR vecVelocity)
{
	CHECK_INIT();
	int idx;
	cell ret = 1;
...
int CGameMode::OnPlayerEditAttachedObject(cell playerid, cell index, cell response, OBJECT_EDIT_DATA* pEditData)
{
	int idx;
	cell ret = 0;
	...
	if (!amx_FindPublic(&m_amx, "OnPlayerEditAttachedObject", &idx))
	{
		...
		amx_Push(&m_amx, pEditData->iBoneID);
		amx_Push(&m_amx, pEditData->iModelID);
...
int CGameMode::OnClientCheckResponse(cell playerid, cell type, cell addr, cell data)
{
	int idx;
	cell ret = 0;
...
int CGameMode::OnPlayerWeaponShot(cell playerid, cell weaponid, cell hittype, cell hitid, VECTOR* vecPos)
{
	int idx;
	cell ret = 1;
